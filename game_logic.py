...
class GameState:
    ...
    def start_moving(self, pid: str, direction: str) -> bool:
        ...
        # Fixed: DIR_VECT keys no longer have spaces
        ...
        # Moving outward from border (start trail)
        ...
        while p.moving:
            try:
                ...
                async with self.lock:
                    ...
                    # 1. Moving inside own land (no trail)
                    if current_is_land and next_is_land:
                        p.pos = (nx, ny)
                        await self._broadcast()
                        continue

                    # 2. First step from land to empty cell (start trail)
    ...
    async def _capture_enclosed(self, p: Player, land_cell: Tuple[int, int]):
        # land_cell is where the player now stands (inside land)
        ...
        # FIXED: Removed 'async with self.lock' to prevent deadlock.
        # The caller (_move_loop) already holds the lock.
    ...
    def get_game_state(self):
        ...
        for p in self.players.values():
            ...
            for (x, y) in p.land:
                grid[f"{x},{y}"] = p.color
            # Path cells: FIXED - put suffix in VALUE so client can detect it
            for (x, y) in p.path:
